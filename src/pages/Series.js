...
const Series = () => {
  const[movies,setmovies]=useState([])
    ...
    useEffect(()=>{
        axios.get(`https://api.themoviedb.org/3/tv/popular?api_key=41ee83dfa758f13481741c2dd740f286&page=${page}`).then((res)=>setmovies(res.data.results))
        return()=>{
          setmovies([])
      }
      },[page])
  return (<>
    <div className={styles.allbox}>
            <div className={styles.box} style={{backgroundColor:!mode?'rgba(0,0,0,0.8)':'rgba(255,255,255,0.7'}}>
                <div className={styles.top}><h1 style={{color:mode?'black':'white'}}>Series</h1><button style={{backgroundColor:!mode?'black':'white',color:!mode?'white':'black'}}>Page {page}</button></div>
                <div className={styles.movie}>
                        {movies.map((mov,index)=>{return<CardS key={index} {...mov}/>})}
                </div>
