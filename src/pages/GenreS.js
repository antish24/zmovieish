...
const GenreS = () => {
  ...
  const[click,setclick]=useState(10759)
  ...
  useEffect(()=>{
    axios.get('https://api.themoviedb.org/3/genre/tv/list?api_key=41ee83dfa758f13481741c2dd740f286&language=en-US').then((res)=>setgenrename(res.data.genres))
  })
  useEffect(()=>{
    axios.get(`https://api.themoviedb.org/3/discover/tv?api_key=41ee83dfa758f13481741c2dd740f286&language=en-US&sort_by=popularity.desc&page=${page}&with_genres=${genreid}`).then((res)=>setgenre(res.data.results))
    return()=>{
      setgenre([])
  }
  },[genreid,page])
  return (<>
    <div className={styles.box} style={{backgroundColor:!mode?'rgba(0,0,0,0.8)':'rgba(255,255,255,0.7'}}>
      ...
      <div className={styles.movie}>
        <div className={styles.top}><h2 style={{color:mode?'black':'white'}}>{name} Series</h2><button style={{backgroundColor:!mode?'black':'white',color:!mode?'white':'black',boxShadow:mode?'3px 3px 7px black':'3px 3px 8px white'}}>Page {page}</button></div>
        <div className={styles.cardbox}>
            {genre.map((mov,ind)=>{return<CardS key={ind} {...mov}/>})}
        </div>
      </div>
      </div>
